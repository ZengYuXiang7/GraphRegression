...
# executing.
NASBENCH_TFRECORD = "data/nasbench101/nasbench_full.tfrecord"
...
def gen_data_point(nasbench):
    ...
    for unique_hash in nasbench.hash_iterator():
        ...
        for repeat_index in range(len(computed_metrics[epoch])):
            assert (
                len(computed_metrics[epoch]) == 3
            ), "len(computed_metrics[epoch]) should be 3"
        ...
        model_spec = ModelSpec(
            fixed_metrics["module_adjacency"], fixed_metrics["module_operations"]
        )
        data = nasbench.query(model_spec, epochs=108)
        print("training time: {}".format(data["training_time"]))
        ...
        if len(ops) < min_l:
            min_l = len(ops)
        print(
            "best val. acc: {:.4f}, best test acc {:.4f}, min length of nodes {:d}".format(
                best_val_acc, best_test_acc, min_l
            )
        )
        ...
        yield {
            i: {  # unique_hash
                ...
                "module_adjacency": adj_array,  # list
                #'module_operations': ops_arry.tolist(), #list
...
def gen_json_file():
    nasbench = NASBench(NASBENCH_TFRECORD)
    nas_gen = gen_data_point(nasbench)
    data_dict = OrderedDict()
    for data_point in nas_gen:
        data_dict.update(data_point)
    with open("data/nasbench101/nasbench101.json", "w") as outfile:
