            ...
            if check_version(in_kwargs["opsets"], out_kwargs["opsets"]):
                ...
                if attr_name not in in_kwargs:
                    if "default" not in out_kwargs:
                        ...
                        raise Exception(
                            "ATTR {} <opsets={}> is required while constructing!".format(
                                attr_name, in_kwargs["opsets"]
                            )
                        )
...
def check_redundant_attrs(attrs, class_name=""):
    ...
    if (not attrs) is False:
        WARNINGS.add(
            "Warnings: unused args for <{}> in opsets <{}>: {}!".format(
                class_name, tv, attrs
            )
        )
...
class AttrClip(AttrBaseOp):
    ...
    @ATTR(name="max", opsets=(1, 11), default=3.402823e38)  # v1 add, v11 delete
    @ATTR(name="min", opsets=(1, 11), default=-3.402823e38)  # v1 add, v11 delete
    def __init__(self, name="", **kwargs):
...
class AttrConstant(AttrBaseOp):
    ...
    @ATTR(
        name="sparse_value", opsets=(11,), default=None
    )  # v11 add, record dims not value
...
class AttrConvTranspose(AttrBaseOp):
    ...
    @ATTR(name="dilations", opsets=(1,), default=1, fmt_func=FSize.dilation)  # v1 add
    @ATTR(
        name="output_padding", opsets=(1,), default=0, fmt_func=FSize.padding
    )  # v1 add
    @ATTR(name="kernel_shape", opsets=(1,), fmt_func=FSize.kernel)  # v1 add, required
    def __init__(self, input_channel, output_channel, bias, name="", **kwargs):
...
class AttrResize(AttrBaseOp):
    ...
    @ATTR(
        name="coordinate_transformation_mode", opsets=(11,), default="half_pixel"
    )  # v10 add
