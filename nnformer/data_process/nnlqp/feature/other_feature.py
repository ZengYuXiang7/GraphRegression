...
def extract_topology_feature(
    nx_G, name2id, id2name, embed_type="nerf", input_type="np_array"
):
...
def extract_depthtoken_feature(depth, embed_type="nerf", input_type="np_array"):
    ...
    total_dim = sum(dims)
    fn = Embedder(total_dim // 2, embed_type, input_type)
...
def extract_end_feature(end=1e5, embed_type="nerf", input_type="np_array"):
    ...
    for part in list(FEATURE_LENGTH.keys())[:-1]:
        for elem in range(FEATURE_LENGTH[part]):
            fn = Embedder(
                FEATURE_DIM[part] // FEATURE_LENGTH[part] // 2, embed_type, input_type
            )
