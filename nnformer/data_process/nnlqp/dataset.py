...
def get_torch_data(onnx_file, batch_size, cost_time, embed_type):
    (
        ...
        end_feature,
        depth_feature,
    ) = extract_graph_feature(onnx_file, batch_size, embed_type)
    node_feats = torch.from_numpy(np.array(node_features, dtype=np.float32)).type(
        torch.float
    )
    topo_feats = torch.from_numpy(np.array(topo_features, dtype=np.float32)).type(
        torch.float
    )
    static_feats = torch.from_numpy(np.array(static_features, dtype=np.float32)).type(
        torch.float
    )
    end_feats = (
        torch.from_numpy(np.array(end_feature, dtype=np.float32))
        .type(torch.float)
        .view(1, -1)
    )
    depth_feats = (
        None
        if depth_feature.all() == None
        else torch.from_numpy(np.array(depth_feature, dtype=np.float32))
        .type(torch.float)
        .view(1, -1)
    )
    ...
    features = (
        torch.cat([features, end_feats], dim=0)
        if depth_feats.all() == None
        else torch.cat([features, end_feats, depth_feats], dim=0)
    )
            ...
            for line in f.readlines():
                ...
                if os.path.exists(onnx_file):
                    ...
                    sf_file = os.path.join(
                        self.data_root, "{}_{}_sf.pt".format(speed_id, plt_id)
                    )
                    ...
                    data, sf = get_torch_data(
                        GG, batch_size, cost_time, self.embed_type
