...
def custom_shape_infer(onnx_G):
    ...
    vals = [
        x for x in onnx_G.graph.value_info
    ]  # onnx.graph.value_info: type and outshape of nodes except for input and output
    for x in vals:
    ...
    for val in onnx_G.graph.input:  # Parse data
        ...
        if val.name in init_dtypes and dtype != init_dtypes[val.name]:
            print(
                " - Warning: {}: init type:{} vs input type:{}".format(
                    val.name, init_dtypes[val.name], dtype
                )
            )
    ...
    if onnx.__version__ not in vers:
        print(
            " - Warning: onnx version should in {}, but with {}, infer shape may cause error".format(
                vers, onnx.__version__
            )
        )
    ...
    input_tensor_new = onnx.helper.make_tensor_value_info(
        ...
        elem_type=1,
        shape=[
            input_shape[0].dim_value,
            input_shape[1].dim_value,
            input_shape[2].dim_value,
            input_shape[3].dim_value,
        ],
    )
    ...
    for i, tensor in enumerate(tensors):
        ...
        tmp_onnx_G.graph.input.insert(
            i + 1, value_info
        )  # because 0 is for placeholder, so start index is 1
    ...
    for val in G.graph.value_info:  # Parse each node，recode its name and shape
        output_shapes[val.name] = parse_dim(
            val
        )  # e.g. resnetv15_pool1_fwd: [8, 512, 1, 1]
    ...
    for k, v in miss_ops.items():
        print(
            " - Warning: Miss shape infer, op=[{}], name=[{}], output=[{}]!".format(
                k, v[0], v[1]
            )
        )
