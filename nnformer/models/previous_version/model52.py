...
class Net(nn.Module):
    def __init__(self, config):
        ...
        # 添加入度和出度的Linear层（onehot编码）
        self.in_degree_linear = nn.Linear(32, self.d_model)
        self.out_degree_linear = nn.Linear(32, self.d_model)

        # 融合所有特征的Linear层（4*d_model -> d_model）
        self.feature_fusion = nn.Linear(4 * self.d_model, self.d_model)
    ...
    def get_data(self, sample, static_feature):
        ...
        # 从邻接矩阵直接计算入度和出度
        in_degree = adj.sum(dim=1).long()  # [B, L] 每个节点的入度
        out_degree = adj.sum(dim=2).long()  # [B, L] 每个节点的出度
    ...
    def forward(self, sample, static_feature):
        ...
        # 节点特征嵌入：各个特征分别编码
        x_op = self.op_embeds(x)  # [B, L, d_model]
        x_depth = self.depth_embed(depth)  # [B, L, d_model]

        # 将入度和出度转换为onehot编码（维度32），然后通过Linear层
        in_degree_onehot = F.one_hot(in_degree.clamp(max=31), num_classes=32).float()
        out_degree_onehot = F.one_hot(out_degree.clamp(max=31), num_classes=32).float()
        x_in_degree = self.in_degree_linear(in_degree_onehot)  # [B, L, d_model]
        x_out_degree = self.out_degree_linear(out_degree_onehot)  # [B, L, d_model]

        # 将所有特征拼接然后融合
        x = torch.cat([x_op, x_depth, x_in_degree, x_out_degree], dim=-1)  # [B, L, 4*d_model]
        x = self.feature_fusion(x)  # [B, L, d_model]
